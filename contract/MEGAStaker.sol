...
contract MEGAStaker {
    event Staking(address indexed staker, uint256 amount, uint256 time);
    ...
    function staking(uint256 amount) external {
        require(token.allowance(msg.sender, address(this)) >= amount, "Need Approve ERC20 token");	
    ...
    function currentRewards(address staker) public view returns (uint256) {
    ...
    function currentAmount(address staker) public view returns (uint256) {
        StakeInfo storage stakeInfo = stakes[staker];
